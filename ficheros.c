...
int mi_write_f(unsigned int ninodo, const void *buf_original, unsigned int offset, unsigned int nbytes) {
  ...
  if (leer_inodo(ninodo, &inodo) < 0) {
    fprintf(stderr, "Error en ficheros.c mi_write_f() 1--> %d: %s\n", errno, strerror(errno));
    return -1;
  }
  if ((inodo.permisos & 2) == 2) {
    ...
    if (primerBLogico == ultimoBLogico) {
      ...
      if (bfisico == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 2--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
      if (bread(bfisico, buf_bloque) == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 3--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
      memcpy(buf_bloque+desp1, buf_original, nbytes);
      if (bwrite(bfisico, buf_bloque) == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 4--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
      ...
      bfisico = traducir_bloque_inodo(ninodo, primerBLogico, 1);
      if (bfisico == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 5--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
      if (bread(bfisico, buf_bloque) == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 6--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
      desp1 = offset % BLOCKSIZE;
      memcpy(buf_bloque+desp1, buf_original, BLOCKSIZE - desp1);
      if (bwrite(bfisico, buf_bloque) == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 7--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
      // Bloques intermedios
      //printf("PrimerBLogico: %d, ultimoBLogico: %d\n", primerBLogico, ultimoBLogico);
      for (int i = primerBLogico + 1; i < ultimoBLogico; i++) {
        bfisico = traducir_bloque_inodo(ninodo, i, 1);
        //printf("Ninodo: %d, NBlogico: %d, Nbfisico: %d\n", ninodo, i, bfisico);
        if(bwrite(bfisico, buf_original + (BLOCKSIZE - desp1) + (i - primerBLogico - 1) * BLOCKSIZE) == -1) {
          fprintf(stderr, "Error en ficheros.c mi_write_f() 8--> %d: %s\n", errno, strerror(errno));
          return -1;
        }
      }
      // Último bloque
      ...
      if (bfisico == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 9--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
      if (bread(bfisico, buf_bloque) == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 10--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
      desp2 = (offset + nbytes - 1) % BLOCKSIZE;
      memcpy(buf_bloque, buf_original + nbytes - desp2 - 1, desp2 + 1);
      if (bwrite(bfisico, buf_bloque) == -1) {
        fprintf(stderr, "Error en ficheros.c mi_write_f() 11--> %d: %s\n", errno, strerror(errno));
        return -1;
      }
    }
    if (leer_inodo(ninodo, &inodo) < 0) {
      fprintf(stderr, "Error en ficheros.c mi_write_f() 12--> %d: %s\n", errno, strerror(errno));
      return -1;
    }
    ...
    if (escribir_inodo(ninodo, inodo) == -1) {
      fprintf(stderr, "Error en ficheros.c mi_write_f() 13--> %d: %s\n", errno, strerror(errno));
      return -1;
    }
  } else {
    fprintf(stderr, "Error en ficheros.c mi_write_f() 14-> No tienes permisos para escribir --> %d: %s\n", errno, strerror(errno));
    return -1;
  }
  return nbytes;
}
...
int mi_read_f(unsigned int ninodo, void *buf_original, unsigned int offset, unsigned int nbytes) {
  ...
  if ((inodo.permisos & 4) == 4) {
    ...
    if (escribir_inodo(ninodo, inodo) == -1) {
      fprintf(stderr, "Error 11 en ficheros.c mi_read_f() --> %d: %s\n", errno, strerror(errno));
