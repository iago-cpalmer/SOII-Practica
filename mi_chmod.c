...
int main(int argc, char **argv) {
    ...
    if (argv[1] == NULL || argv[2] == NULL || argv[3] == NULL) {
        fprintf(stderr,"Paràmetros no especificados. Uso: mi_chmod <nombre_dispositivo> <permisos> </ruta>\n");
        exit(1);
    }
    if(bmount(argv[1]) ==-1) {
        fprintf(stderr,"mi_chmod --> Error al montar el dispositivo\n");
        return -1;
    }
    ...
    int x = mi_chmod(argv[3], permisos);
    if (bumount() == -1) {
